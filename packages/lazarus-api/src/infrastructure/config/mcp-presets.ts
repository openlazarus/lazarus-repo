...
export const MCP_PRESETS: Record<string, MCPPreset> = {
  ...
  canva: {
    ...
    // complete from arbitrary hosts today. End users who want Canva should
    // connect it from a local MCP client (Claude Desktop, Cursor, etc.) via
    // mcp-remote on localhost. To enable hosted use, Canva must add the host
    // to the mcp.canva.com allowlist (vendor request).
    ...
    // mcp.canva.com/authorize enforces a host allowlist, so hosted deployments
    // typically can't complete OAuth from the UI today. The preset is hidden
    // behind "Coming soon" until either Canva allowlists the host or a Canva
    // Connect-based MCP server replaces it. Previously-connected workspaces
    // keep functioning because their tokens are saved on disk and their
    // .mcp.config.json entry is preserved.
    comingSoon: true,
  },
